      ...
      while (hasMorePages && currentPage <= totalPages) {
        ...
        if (result.statusCode === 200) {
          ...
          if (result.data && Array.isArray(result.data.transactions)) {
            transactionsArray = result.data.transactions;
          } else if (result.data && Array.isArray(result.data)) {
            transactionsArray = result.data;
          ...
          if (transactionsArray.length > 0) {
            const filteredTransactions = transactionsArray.filter((transaction: Transaction) => 
              allowedTypes.includes(transaction.type)
            );
          ...
          // Update pagination
          totalPages = result.pagination?.totalPages || result.data?.pagination?.totalPages || 1;
          hasMorePages = result.pagination?.hasNextPage || result.data?.pagination?.hasNextPage || false;
          currentPage++;
        } else {
...
const filteredTransactions = useMemo(() => {
  return transactions.filter((transaction) => {
    ...
    // Fix: Make status matching case-insensitive
    const matchesStatus =
      filterStatus === "All" || 
