...
const CreateTrade = ({
  ...
  onTradeCreated,
  preSelectedTraderId,
}: CreateTradeComponentProps) => {
  const [traderId, setTraderId] = useState(preSelectedTraderId || "");
  ...
  useEffect(() => {
    if (preSelectedTraderId) {
      setTraderId(preSelectedTraderId);
    }
  }, [preSelectedTraderId]);
  ...
  const handelSumbit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const tradeData: CreateTradeProps = {
        ...
        leverage,
        orderType,
        notes: notes.trim() || `Admin created ${side} trade for ${tradePairFull}`,
      };
      ...
      if (response.status === 201) {
        ...
        // Call both callbacks with proper data
        onTradeCreated?.();
      ...
      console.log('Trade created successfully, closing modal and refreshing data');
    } catch (err) {
          ...
          <div className="flex space-x-3">
            ...
            <div className="w-1/2">
              ...
              <select
                ...
                className="mt-1 block w-full rounded-md bg-[#10131F] border border-gray-700 text-white shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm p-2"
              >
                <option value="MARKET" className="bg-[#10131F] text-white">
                  Market
                </option>
                <option value="LIMIT" className="bg-[#10131F] text-white">
                  Limit
                </option>
                <option value="STOP_LIMIT" className="bg-[#10131F] text-white">
                  Stop-Limit
                </option>
              </select>
            </div>
